...
//setup for points
GLfloat pointMat_ambient[] = { 0.0, 0.0, 0.0, 1.0 };
GLfloat pointMat_specular[] = { 0.45, 0.55, 0.45, 1.0 };
GLfloat pointMat_diffuse[] = { 0.05, 0.05, 0.05, 1.0 };
GLfloat pointMat_shininess[] = { 100.0 };

GLfloat lineMat_ambient[] = { 0.0, 0.0, 0.0, 1.0 };
GLfloat lineMat_specular[] = { 0.45, 0.55, 0.45, 1.0 };
GLfloat lineMat_diffuse[] = { 0.2, 0.6, 0.2, 1.0 };
GLfloat lineMat_shininess[] = { 10.0 };

GLfloat normalMat_ambient[] = { 0.0, 0.0, 0.0, 1.0 };
GLfloat normalMat_specular[] = { 0.45, 0.55, 0.45, 1.0 };
GLfloat normalMat_diffuse[] = { 0.2, 0.2, 0.6, 1.0 };
GLfloat normalMat_shininess[] = { 10.0 };
...
int numVertices;
GLuint* indices;
boolean VBOallocated = false;

void buildVBOs() {
	if (!VBOallocated) {
		//create vertices array same length as varray * 3 for each x,y,z position
		numVertices = subcurve.numCurvePoints * NUMBEROFSIDES * 3;
		vertices = (double*)malloc(numVertices * sizeof(double));
		normals = (double*)malloc(numVertices * sizeof(double));

		numIndices = (subcurve.numCurvePoints - 1) * NUMBEROFSIDES * 4;
	...
	for (int row = 0; row < subcurve.numCurvePoints; row++)
	{
		for (int col = 0; col < NUMBEROFSIDES; col++)
		{
			vertices[vidx] = varray[row * NUMBEROFSIDES + col].x;
			vertices[vidx + 1] = varray[row * NUMBEROFSIDES + col].y;
			vertices[vidx + 2] = varray[row * NUMBEROFSIDES + col].z;
			vidx += 3;


			normals[nidx] = varray[row * NUMBEROFSIDES + col].normal.x;
			normals[nidx + 1] = varray[row * NUMBEROFSIDES + col].normal.y;
			normals[nidx + 2] = varray[row * NUMBEROFSIDES + col].normal.z;
			nidx += 3;

			if (row < subcurve.numCurvePoints - 1) {
				indices[iidx] = qarray[row * NUMBEROFSIDES + col].vertexIndex[0];
				indices[iidx + 1] = qarray[row * NUMBEROFSIDES + col].vertexIndex[1];
				indices[iidx + 2] = qarray[row * NUMBEROFSIDES + col].vertexIndex[2];
				indices[iidx + 3] = qarray[row * NUMBEROFSIDES + col].vertexIndex[3];
				iidx += 4;
			}
		}
	}
...
{
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, pointMat_ambient);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, pointMat_specular);
	glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, pointMat_diffuse);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, pointMat_shininess);
...
void drawQuadsAsLines() 
{
	...
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, lineMat_ambient);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, lineMat_specular);
	glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, lineMat_diffuse);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, lineMat_shininess);
	...
	for (int row = 0; row < subcurve.numCurvePoints - 1; row++)
	{
		for (int col = 0; col < NUMBEROFSIDES; col++)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				int j = (i + 1) % 4;
				glBegin(GL_LINES);
				Vertex* vertex1 = &varray[qarray[row * NUMBEROFSIDES + col].vertexIndex[i]];
				Vertex* vertex2 = &varray[qarray[row * NUMBEROFSIDES + col].vertexIndex[j]];
	...
	if (drawNormals)
	{
		glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, normalMat_ambient);
		glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, normalMat_specular);
		glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, normalMat_diffuse);
		glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, normalMat_shininess);
...
//code to export mesh to file outputMesh.txt
void exportMesh() {
	
	FILE *fp;

	if (fopen_s(&fp, "outputMesh.txt", "w") != 0) {
